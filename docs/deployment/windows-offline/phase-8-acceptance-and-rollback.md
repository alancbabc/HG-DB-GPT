...
每个文件记录大小和SHA-256。恢复始终写入一个不存在的新目录，绝不覆盖、删除或重命名当前数据目录：
...
| 离线发布组包、哈希和篡改检测 | 假介质测试通过 |
| 运行数据备份、校验、恢复到新目录 | 临时数据测试通过 |
| 五粮液数据库与C++模拟宿主联合24/72小时 | 120秒并发短测通过；宿主不可用提示及恢复重连单测通过；长测待目标环境 |
| Windows服务实际安装、自启动、恢复 | 待目标环境 |
| RTX 3090真实LLM与Embedding | 待模型介质和目标机 |
| 完全断网全新安装 | 待完整发布介质和干净目标机 |
